...
import (
	...
	"github.com/moroshma/benchmarks/kafka/pkg/metrics"
)
...
func main() {
	...
	fmt.Printf("Starting Kafka benchmark: %s\n", config.Test.Name)
	...
	result := collector.Finalize("kafka", config.Test.Name)
...
func createTopic(config *Config) error {
	...
	admin, err := sarama.NewClusterAdmin(config.Kafka.Brokers, kafkaConfig)
	if err != nil {
		return err
	}
	defer admin.Close()
	...
	err = admin.CreateTopic(config.Kafka.Topic, topicDetail, false)
	if err != nil {
...
func runProducer(
	...
	kafkaConfig := sarama.NewConfig()
	kafkaConfig.Version = sarama.V2_8_0_0
	kafkaConfig.Producer.RequiredAcks = sarama.RequiredAcks(config.Kafka.Producer.RequiredAcks)
	kafkaConfig.Producer.Compression = parseCompression(config.Kafka.Producer.Compression)
	kafkaConfig.Producer.MaxMessageBytes = config.Kafka.Producer.MaxMessageBytes
	kafkaConfig.Producer.Timeout = config.Kafka.Producer.Timeout
	...
	for i := int64(0); i < numMessages; i++ {
		...
		msg := &sarama.ProducerMessage{
			...
			Value: sarama.ByteEncoder(testData),
			Headers: []sarama.RecordHeader{
				{
					Key:   []byte("producer_id"),
					Value: []byte(fmt.Sprintf("%d", producerID)),
				},
				{
					Key:   []byte("message_id"),
...
func parseCompression(comp string) sarama.CompressionCodec {
	switch comp {
	case "gzip":
