...
# Undeploy MiniToolStreamIngress from k3s
...
echo -e "${BLUE}=== MiniToolStreamIngress Undeployment ===${NC}"
...
# Confirm deletion
read -p "Are you sure you want to delete MiniToolStreamIngress deployment? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Aborted"
    exit 0
